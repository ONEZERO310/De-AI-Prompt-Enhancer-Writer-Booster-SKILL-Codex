...
本仓库里的 `de-AI-writing` 和 `good-writing` 已按轻量读取方式整理。Codex 调用时应优先读取各 Skill 的 `SKILL.md`，再按入口文件中的“读取策略”加载短索引或摘要。

## 安装位置确认

如果 Codex 实际读取的是全局技能目录，需要确认以下位置是否和本仓库同步：
...
当前仓库只负责维护仓库内两份 Skill。若 Codex 仍卡在 Read，请先确认它加载的不是旧的全局副本。
